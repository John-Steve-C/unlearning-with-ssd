...
class CustomTrainer(Trainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        # for LLaMa-2, the condition should be if "embed_tokens" not in name
        selected_named_parameters = [(name, param) for name, param in self.model.named_parameters() if "wte" not in name and "wpe" not in name]
        self.param_num = len(selected_named_parameters)
        print(f"selected_named_parameters: {len(selected_named_parameters)}")
...
def dump_importance(importances, pkl_file_path):
    ...
    def step(self, closure=None):
        ...
        for idx, group in enumerate(self.param_groups):
            ...
            for i, p in enumerate(group['params']):
                ...
                importance[i] += torch.norm(p.grad.data.clone(), 2)
...
def train():
    ...
    trainer = CustomTrainer(model=model, tokenizer=tokenizer, args=training_args, mask=None, train_dataset=train_dataset, eval_dataset=test_dataset)
    trainer.train()
    # trainer.save_state()
    # trainer.save_model(output_dir=training_args.output_dir)

    global importance  # 声明 importance 是一个全局变量

    importance = importance[:trainer.param_num]
    dump_importance(importance, "grad_importance.pkl")

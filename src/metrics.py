...
def UnLearningScore(tmodel, gold_model, forget_dl, batch_size, device):
    ...
    with torch.no_grad():
        for batch in forget_dl:
            ...
            model_output = tmodel(**b)
            gold_model_output = gold_model(**b)
            model_preds.append(F.softmax(model_output.logits, dim=-1).detach().cpu())
            gold_model_preds.append(F.softmax(gold_model_output.logits, dim=-1).detach().cpu())
...
def actv_dist(model1, model2, dataloader, device="cuda"):
    ...
    for batch in dataloader:
        ...
        diff = torch.sqrt(
            torch.sum(
                torch.square(
                    F.softmax(model1_out.logits, dim=-1) - F.softmax(model2_out.logits, dim=-1)
                ),
                axis=1,
            )
        )
